...
function ameOnDomReady() {
	...
	menuEditorNode.on(
		...
		function(event, selectedItem, column, button) {
			...
			//Sort items in this column and all of the following columns.
			...
			while(currentColumn) {
				...
				currentColumn.getAllItemLists().each(function() {
					sortMenuItems($(this), direction, leaveFirstItem);
				});

				currentColumn = menuPresenter.getColumnImmediate(currentColumn.level + 1);
			}
		}
	);
