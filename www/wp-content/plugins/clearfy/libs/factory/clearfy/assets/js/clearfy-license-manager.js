...
jQuery(function($) {
	...
	$(document).on('click', '.wcl-control-btn', function() {
		...
		for( i = 0; i < allNotices.length; i++ ) {
			$.wbcr_factory_clearfy_239.app.hideNotice(allNotices[i]);
		}
			...
			success: function(response) {
				...
				if( !response || !response.success ) {
					...
					if( response.data ) {
						...
						noticeId = $.wbcr_factory_clearfy_239.app.showNotice('Error: [' + response.data.error_message + ']', 'danger');
				...
				if( response.data && response.data.message ) {
					noticeId = $.wbcr_factory_clearfy_239.app.showNotice(response.data.message, 'success');
					allNotices.push(noticeId);
