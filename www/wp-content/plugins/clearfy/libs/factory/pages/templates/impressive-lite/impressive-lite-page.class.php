...
if ( ! class_exists( 'Wbcr_FactoryPages446_ImpressiveLiteTemplate' ) ) {
	/**
	 * Class Wbcr_FactoryPages446_ImpressiveLiteTemplate
	 */
	abstract class Wbcr_FactoryPages446_ImpressiveLiteTemplate extends Wbcr_FactoryPages446_ImpressiveThemplate {
		...
		public function __construct( Wbcr_Factory447_Plugin $plugin ) {
			$this->menuIcon = FACTORY_PAGES_446_URL . '/templates/impressive-lite/assets/img/webcraftic-plugin-icon.png';
			//$allow_multisite = apply_filters('wbcr_factory_447_core_admin_allow_multisite', false);
			...
			$this->title_plugin_action_link = __( 'Settings', 'wbcr_factory_pages_446' );
		...
		public function assets( $scripts, $styles ) {
			...
			$this->styles->add( FACTORY_PAGES_446_URL . '/templates/impressive-lite/assets/css/impressive-lite.page.template.css' );
		}
		...
		public function flushCacheAndRulesAction() {
			...
			if ( class_exists( 'WbcrFactoryClearfy239_Helpers' ) ) {
				WbcrFactoryClearfy239_Helpers::flushPageCache();
			}
		...
		protected function warningNotice() {
			/*if( WP_CACHE ) {
				$this->printWarningNotice(__("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_446'));
			}*/
		...
		private function showActionsNotice() {
			$notices = [
				[
					...
					'type'       => 'success',
					'message'    => __( 'The settings have been updated successfully!', 'wbcr_factory_pages_446' ) . ( WP_CACHE ? '<br>' . __( "It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_446' ) : '' )
				]
			];
                ...
                <div class="wbcr-factory-control">
					...
					<?php if ( $this->type == 'options' ): ?>
                        <input name="<?= $this->plugin->getPluginName() ?>_save_action"
                               ...
                               value="<?php _e( 'Save', 'wbcr_factory_pages_446' ); ?>">
		...
		protected function showOptions() {
			$form = new Wbcr_FactoryForms444_Form( [
				'scope' => rtrim( $this->plugin->getPrefix(), '_' ),
				'name'  => $this->getResultId() . "-options"
			], $this->plugin );

			$form->setProvider( new Wbcr_FactoryForms444_OptionsValueProvider( $this->plugin ) );
			...
			if ( isset( $_POST[ $this->plugin->getPluginName() . '_save_action' ] ) ) {
				...
				if ( ! current_user_can( 'administrator' ) && ! current_user_can( $this->capabilitiy ) ) {
					wp_die( __( 'You do not have permission to edit page.', 'wbcr_factory_pages_446' ) );
				}
            ...
            <div id="WBCR" class="wrap">
                <div class="wbcr-factory-pages-446-impressive-lite-page-template factory-bootstrap-447 factory-fontawesome-000">
