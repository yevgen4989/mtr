<?php
	...
	if( !class_exists('Wbcr_FactoryForms444_ListControl') ) {

		class Wbcr_FactoryForms444_ListControl extends Wbcr_FactoryForms444_Control {
				...
				if( $this->isAjax() ) {
					...
					<script>
						window['<?php echo $ajax_id ?>'] = {
							...
							'emptyList': '<?php echo $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_444') ) ?>'
						};
					</script>
					<?php
			...
			protected function checklistHtml()
			{
				...
				$emptyList = $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_444'));
