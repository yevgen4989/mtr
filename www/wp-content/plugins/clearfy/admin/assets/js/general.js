...
(function($) {
	...
	var general = {
		init: function() {
			...
			/*$.wfactory_447.hooks.add('core/components/updated', function(button, data, response) {
				// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
				if( !($('#WBCR').length && $.wbcr_factory_clearfy_239) ) {
					return false;
				}
				if( response.data.need_rewrite_rules && !$('.wbcr-clr-need-rewrite-rules-message').length ) {
					$.wbcr_factory_clearfy_239.app.showNotice(response.data.need_rewrite_rules, 'warning');
				}
			});*/
		},
			...
			$('.wbcr-clearfy-popup-button-ok').click(function() {
				...
				self.sendRequest({
						...
						if( !data || data.error ) {
							 ...
							 * $.wbcr_factory_clearfy_239.hooks.add('wbcr/factory_clearfy_239/updated',
							 * function(noticeId) {});
							 * @param {string} modeName - имя режима конфигурации
							 * @param {object} data
							 */

							$.wbcr_factory_clearfy_239.hooks.run('clearfy/quick_start/configurated_error', [
								modeName,
								data
							]);
							return;
					...
					function(data) {
						 ...
						 * libs/clearfy/admin/assests/js/global.js Пример регистрации хука
						 * $.wbcr_factory_clearfy_239.hooks.add('wbcr/factory_clearfy_239/updated', function(noticeId)
						 * {});
						 * @param {string} modeName - имя режима конфигурации
						 * @param {object} data
						 */
						$.wbcr_factory_clearfy_239.hooks.run('clearfy/quick_start/configurated', [modeName, data]);
		...
		importOptions: function() {
			...
			$('.wbcr-clearfy-import-options-button').click(function() {
				...
				if( !settings ) {
					$.wbcr_factory_clearfy_239.app.showNotice('Import options is empty!', 'danger');
					return false;
				}

				if( void 0 === wbcr_clearfy_ajax || !wbcr_clearfy_ajax.import_options_nonce ) {
					$.wbcr_factory_clearfy_239.app.showNotice('Unknown Javascript error, most likely the wbcr_clearfy_ajax variable does not exist!', 'danger');
					return false;
				}
				...
				self.sendRequest({
					...
					if( response.data.update_notice ) {
						if( !$('.wbcr-clr-update-package').length ) {
							$.wbcr_factory_clearfy_239.app.showNotice(response.data.update_notice);
						}
					} else {
						if( $('.wbcr-clr-update-package').length ) {
			...
			$.ajax(ajaxurl, {
				...
				success: function(data, textStatus, jqXHR) {
					...
					if( !data || data.error ) {
						...
						if( data ) {
							noticeId = $.wbcr_factory_clearfy_239.app.showNotice(data.error_message, 'danger');
						} else {
							if( void 0 !== wbcr_clearfy_ajax ) {
								noticeId = $.wbcr_factory_clearfy_239.app.showNotice(wbcr_clearfy_ajax.i18n.unknown_error, 'danger');
							}
						}

						setTimeout(function() {
							$.wbcr_factory_clearfy_239.app.hideNotice(noticeId);
						}, 5000);
						return;
					}
					...
					if( !request_data.flush_redirect ) {
						if( void 0 !== wbcr_clearfy_ajax ) {
							noticeId = $.wbcr_factory_clearfy_239.app.showNotice(wbcr_clearfy_ajax.i18n.success_update_settings, 'success');
				...
				error: function(xhr, ajaxOptions, thrownError) {
					...
					var noticeId = $.wbcr_factory_clearfy_239.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
				}
			});
		}
	};
